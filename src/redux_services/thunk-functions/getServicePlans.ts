...
export const getServicePlans = (serviceID: string) => async (dispatch: AppDispatch) => {
  try {
    ...
    const servicePlans = await checkResponse(response);
    dispatch(getServicePlansSuccess(servicePlans));
  } catch (err) {
